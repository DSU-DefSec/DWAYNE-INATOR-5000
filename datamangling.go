...
func processTeamRecord(rec teamRecord, mux *sync.Mutex) {
	...
	if len(old.Checks) != len(rec.Checks) {
		fmt.Println("error copying over check values: length difference. length of old checks is", len(old.Checks), "while new is", len(rec.Checks))
	} else {
	...
	for i := range rec.Checks {
		if rec.Checks[i].Status {
			...
			if rec.Checks[i].SlaCounter >= mewConf.SlaThreshold {
				fmt.Println("triggered SLA violation!")
				rec.Checks[i].SlaCounter = 0
				rec.Checks[i].SlaViolations++
			} else {
				fmt.Println("status was NO for team", rec.Team.Name, "check", rec.Checks[i].Name, "going from", rec.Checks[i].SlaCounter, "to", rec.Checks[i].SlaCounter + 1)
